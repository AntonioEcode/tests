...
$copy_author        = __( 'Por', 'frontecode' );
$author_image_src   = get_user_meta( $author_id, '_user_profile_image', TRUE );
$author_image_src   = !empty( $author_image_src ) ? $author_image_src : default_image_post( 'url' );
...
// Related posts
$related_posts_title = get_post_meta( $post_id, '_{{template_name}}_related_posts_title_{{page_section_id}}', TRUE );
$posts_blog = get_post_meta( $post_id, '_{{template_name}}_related_posts_blog_{{page_section_id}}', TRUE );
...
<section id="page_section_{{page_section_id}}"{{builder}} class="page_section"{{/builder}}>
	<section class="ecode_section_153_template_247">
		<div id="ecode_width_153_247" class="ecode_width_153_247">
			<section class="ecode_post">
				<p class="ecode_time_comments">
					<time class="ecode_time">{{format_post_date}}</time> ·
					<span class="ecode_comments">{{copy_number_comments}}</span>
				</p>
				<picture class="ecode_image ecode_false_link" data-link="h3"{{builder}} data-edit="ecode_article_image"{{/builder}}>
				...
				<div class="ecode_container_content">
					...
					<p>Meh synth Schlitz, tempor duis single-origin coffee ea next level ethnic fingerstache fanny pack nostrud. Photo booth anim 8-bit hella, PBR 3 wolf moon beard Helvetica. Salvia esse nihil, flexitarian Truffaut synth art party deep v chillwave. Seitan High Life</p>
					...
					<p>Meh synth Schlitz, tempor duis single-origin coffee ea next level ethnic fingerstache fanny pack nostrud. Photo booth anim 8-bit hella, PBR 3 wolf moon beard Helvetica. Salvia esse nihil, flexitarian Truffaut synth art party deep v chillwave. Seitan High Life</p>
					<p>Meh synth Schlitz, tempor duis single-origin coffee ea next level ethnic fingerstache fanny pack nostrud. Photo booth anim 8-bit hella, PBR 3 wolf moon beard Helvetica. Salvia esse nihil, flexitarian Truffaut synth art party deep v chillwave. Seitan High Life reprehenderit consectetur cupidatat kogi. Et leggings fanny pack, elit bespoke vinyl art party Pitchfork selfies master cleanse Kickstarter seitan retro. Drinking vinegar stumptown yr pop-up artisan sunt. Deep v cliche lomo biodiesel Neutra selfies. Shorts fixie consequat flexitarian four loko tempor duis single-origin coffee. Banksy, elit small batch freegan sed.</p>
					<blockquote>Meh synth Schlitz, tempor duis single-origin coffee ea next level ethnic fingerstache fanny pack nostrud. Photo booth anim 8-bit hella.</blockquote>{{/builder}}
				</div>
				<section class="ecode_author">
					{{php}}<?php

					if( !empty( $author_image_src ) ) {

					?>{{/php}}
					<figure class="ecode_author_figure">
						<img loading="lazy" src="{{author_image_src}}" alt="{{author_name}}">
					</figure>
					{{php}}<?php
					...
					?>{{/php}}
					<span class="ecode_author_subtitle">{{copy_author}}</span>
					<p class="ecode_author_name">{{author_name}}</p>
					{{php}}<?php

					if( !empty( $author_description ) ) {

					?>{{/php}}
					<p class="ecode_author_desc">{{author_description}}</p>
					{{php}}<?php
				...
				<section class="ecode_related_posts">
					{{php}}<?php

					if( !empty( $related_posts_title ) ) {

					?>{{/php}}
					<p class="ecode_related_posts_title">{{related_posts_title}}</p>
					{{php}}<?php
					...
					<section class="ecode_articles">
						...
						if( !empty( $posts_blog ) ) {
							...
							foreach( $posts_blog as $post ) {
								...
								$related_post_date = new DateTime( $related_post->post_date );
								$related_format_post_date = strftime( "%d de %B, %Y", $related_post_date->getTimestamp() );

								$related_post_thumbnail_id = get_post_thumbnail_id( $related_post_id );

								if( !empty( $related_post_thumbnail_id ) ) {

									$related_post_thumbnail_src = wp_get_attachment_image_src( $related_post_thumbnail_id, 'large' )[0];
									$related_post_thumbnail_alt = get_post_meta( $related_post_thumbnail_id, '_wp_attachment_image_alt', TRUE );
									$related_post_thumbnail_alt = !empty( $related_post_thumbnail_alt ) ? $related_post_thumbnail_alt : $related_post_title;

								} else {

									$related_post_thumbnail_src = default_image_post( 'url' );
									$related_post_thumbnail_alt = $related_post_title;
						...
						<article class="ecode_article">
							<figure class="ecode_article_figure ecode_false_link" data-link="h3">
								<img loading="lazy" src="{{related_post_thumbnail_src}}" alt="{{related_post_thumbnail_alt}}">
							</figure>
							<p class="ecode_article_time_comments">{{related_format_post_date}}</p>
							<h3 class="ecode_article_title"><a href="{{related_post_url}}">{{related_post_title}}</a></h3>
						</article>
						{{posts_loop_end}}<?php
				...
				<section class="ecode_blog_comments">
					<p class="ecode_title">{{copy_comments}}</p>
					<section class="comments">
						...
						<ul class="list_comments">
							<li class="comment even thread-even depth-1">
								<figure><img loading="lazy" alt="" src="https://source.unsplash.com/150x150/?person" srcset="https://source.unsplash.com/150x150/?person" class="avatar avatar-96 photo" height="96" width="96"></figure>
								<div class="name_date">
									<h4>José Martínez</h4>
									<time>2 de febrero de 2021</time>
								</div>
						...
						<section class="form_comment">
							...
							<p class="desc">Tu dirección de correo electrónico no será publicada. Los campos obligatorios están marcados con *</p>
							<form action="#" method="post" id="commentform" class="comment-form">
								...
								<div class="terms_submit">
									<div class="terms">
										...
										<label for="comment_terms"><p>He leído y acepto las Condiciones del servicio y la Política de privacidad</p></label>
									</div>
									<p class="submit ecode_button ecode_button_primary button_disabled">Enviar comentario</p>
								</div>
							</form>
						</section>{{/builder}}
					</section>
			...
			<aside id="ecode_sidebars_153_247" class="ecode_sidebars">
				{{php}}<?php

				dynamic_sidebar( 'ecoded_sidebar_posts' );

				?>{{/php}}{{builder}}<section class="ecode_sidebar ecode_sidebar_posts">
					<h3 class="ecode_sidebar_title">Technology</h3>
					<article>
						<figure class="ecode_false_link" data-link="h3">
							<img src="{{sidebar_image_src}}" alt="Alt image">
						</figure>
						<h3><a href="#">Boston's new hub for robotics startups</a></h3>
					...
					<article>
						...
						<h3><a href="#">OxSight uses augmented reality to aid the visually impaired</a></h3>
						<time>12 Abril, 2017</time>
					</article>
					<article>
						<figure class="ecode_false_link" data-link="h3">
						...
						<h3><a href="#">Tokyo Institute of Technology taps for fastest AI computer</a></h3>
						<time>12 Abril, 2017</time>
					</article>
					<article>
						<figure class="ecode_false_link" data-link="h3">
						...
						<h3><a href="#">Student job finder WayUp acquires competitor Looksharp</a></h3>
						<time>12 Abril, 2017</time>
					</article>
					<article>
						<figure class="ecode_false_link" data-link="h3">
						...
						<h3><a href="#">How Alphabet’s Project Loon balloons learned to loiter</a></h3>
						<time>12 Abril, 2017</time>
					</article>
				</section>
