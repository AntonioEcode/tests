...
<section id="page_section_{{page_section_id}}"{{builder}} class="page_section"{{/builder}}>
	<section class="ecode_section_145_template_215">
		<section class="ecode_curves">
			<svg fill="#ffffff" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1480 142.3" preserveAspectRatio="none"><path d="M704.6 52.7C430.3 1.4 174.9 23 0 43.7v98.7h1480V43.7c-192.5 22.8-441.9 71.3-775.4 9z"></path></svg>
		</section>
		<div class="ecode_width_145_215">
			<section class="ecode_articles ecode_articles_145_215">
				...
				foreach( $slides_group as $slide ) {
					...
					$slide_city          = $slide['slide_city'];
					$slide_place         = $slide['slide_place'];
					...
					$slide_txt_button_3  = $slide['slide_txt_button_3'];
					$slide_link_button_3 = $slide['slide_link_button_3'];
				...
				?>{{/slides_group_loop_start}}
					<article class="ecode_article">
						...
						?>{{/php}}
						<picture class="ecode_article_image"{{builder}} data-edit="ecode_article_image"{{/builder}}>
						...
						if( !empty( $slide_link_button_1 ) && !empty( $slide_txt_button_1 ) ) {

						?>{{/php}}
						<a href="{{slide_link_button_1}}" class="ecode_article_cat"{{builder}} data-edit="ecode_article_cat"{{/builder}}>{{slide_txt_button_1}}</a>
						{{php}}<?php
						...
						?>{{/php}}
						<h1 class="ecode_article_title"{{builder}} data-edit="ecode_article_title"{{/builder}}>{{slide_title}}</h1>
						{{php}}<?php

							} else {

						?>
						<h2 class="ecode_article_title"{{builder}} data-edit="ecode_article_title"{{/builder}}>{{slide_title}}</h2>
						<?php
						...
						if( !empty( $slide_date ) || !empty( $slide_city ) || !empty( $slide_place ) ) {
						...
						<ul class="ecode_article_ul">
							...
							<li class="ecode_article_li"{{builder}} data-edit="ecode_article_li"{{/builder}}>
								...
								<span>{{slide_city}}</span>
							</li>
							{{php}}<?php

							}

							if( !empty( $slide_place ) ) {
						...
						if( ( !empty( $slide_link_button_2 ) && !empty( $slide_txt_button_2 ) ) || ( !empty( $slide_link_button_3 ) && !empty( $slide_txt_button_3 ) ) ) {

						?>{{/php}}
						<div class="ecode_article_buttons">
							{{php}}<?php

							if( !empty( $slide_link_button_2 ) && !empty( $slide_txt_button_2 ) ) {

							?>{{/php}}
							<a href="{{slide_link_button_2}}" class="ecode_article_button ecode_article_button_1"{{builder}} data-edit="ecode_article_button_1"{{/builder}}>{{slide_txt_button_2}}</a>
							{{php}}<?php

							}

							if( !empty( $slide_link_button_3 ) && !empty( $slide_txt_button_3 ) ) {

							?>{{/php}}
							<a href="{{slide_link_button_3}}" class="ecode_article_button ecode_article_button_2"{{builder}} data-edit="ecode_article_button_2"{{/builder}}>{{slide_txt_button_3}}</a>
							{{php}}<?php
