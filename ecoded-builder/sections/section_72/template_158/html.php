...
<section id="page_section_{{page_section_id}}"{{builder}} class="page_section"{{/builder}}>
	<section class="ecode_section_72_template_158"{{builder}} data-edit="ecode_section_72_template_158"{{/builder}}>
		<div class="ecode_width_72_158">
			{{php}}<?php
			...
			<section class="ecode_reviews">
				...
				?>{{/php}}
				<section class="ecode_reviews_slider ecode_reviews_slider_72_158">
				...
				<section class="ecode_reviews_features">
					...
					<article class="ecode_review_feature"{{builder}} data-edit="ecode_review_feature"{{/builder}}>
						...
						?>{{/php}}
						<h3 class="ecode_review_feature_title"{{builder}} data-edit="ecode_review_feature_title"{{/builder}}>{{card_comment}}</h3>
						{{php}}<?php

						if( !empty( $card_name ) ) {

						?>{{/php}}
						<p class="ecode_review_feature_text"{{builder}} data-edit="ecode_review_feature_text"{{/builder}}><span>“</span>{{card_name}}</p>
						{{php}}<?php
	...
	<script type="text/javascript">
		...
		for (var i = 0; i < array_s72_t158.length; i++) {
			array_articles = array_s72_t158[i].querySelectorAll( '.ecode_reviews_slider, .ecode_review_feature' );
