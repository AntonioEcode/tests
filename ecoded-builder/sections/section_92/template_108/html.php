...
$copy_interesting = __( 'Enlaces de interés', 'frontecode' );
$copy_last_posts  = __( 'Últimas noticias', 'frontecode' );
$copy_contact     = __( 'Contacta con nosotros', 'frontecode' );
...
if( $query->have_posts() ) {
	...
	while( $query->have_posts() ) {
		...
		$last_posts[] = array(
			...
			'post_url'           => $post_url,
			'format_post_date'   => $format_post_date,
...
<section id="page_section_{{page_section_id}}"{{builder}} class="page_section"{{/builder}}>
	<footer class="ecode_section_92_template_108"{{builder}} data-edit="ecode_section_92_template_108"{{/builder}}>
		<div class="ecode_width_92_108">
			<section class="ecode_section_footer ecode_info">
				<figure class="logo">
					<a href="{{blog_url}}">
				...
				?>{{/php}}
				<p class="ecode_text"{{builder}} data-edit="ecode_text"{{/builder}}>{{footer_description}}</p>
				{{php}}<?php
			...
			?>{{/php}}
			<section class="ecode_section_footer ecode_posts">
				<p class="ecode_title"{{builder}} data-edit="ecode_title"{{/builder}}>{{copy_last_posts}}</p>
				{{last_posts_group_loop_start}}<?php

				foreach( $last_posts as $post ) {

					$post_title         = $post['post_title'];
					$post_url           = $post['post_url'];
					$format_post_date   = $post['format_post_date'];
					$post_thumbnail_src = $post['post_thumbnail_src'];
					$post_thumbnail_alt = $post['post_thumbnail_alt'];

				?>{{/last_posts_group_loop_start}}
				<article>
					<figure class="ecode_false_link ecode_posts_figure" data-link="h3"{{builder}} data-edit="ecode_posts_figure"{{/builder}}>
						<img loading="lazy" src="{{post_thumbnail_src}}" alt="{{post_thumbnail_alt}}">
					</figure>
					<h3 class="ecode_posts_title"{{builder}} data-edit="ecode_posts_title"{{/builder}}><a href="{{post_url}}">{{post_title}}</a></h3>
					<time class="ecode_posts_date"{{builder}} data-edit="ecode_posts_date"{{/builder}}>{{format_post_date}}</time>
				</article>
				{{last_posts_group_loop_end}}<?php

				}

				wp_reset_postdata();

				?>{{/last_posts_group_loop_end}}
			</section>
			{{php}}<?php
			...
			?>{{/php}}
			<section class="ecode_section_footer ecode_menu">
				<p class="ecode_title"{{builder}} data-edit="ecode_title"{{/builder}}>{{copy_interesting}}</p>
				{{php}}{{menu_nav}}{{/php}}{{builder}}
				<nav class="nav_main_menu">
					<ul class="menu-item ul_main_menu">
						<li class="menu-item menu-item-type-post_type menu-item-object-page menu-item-home current-menu-item page_item current_page_item">
							<a href="#" data-edit="ecode_menu .nav_main_menu .ul_main_menu .menu-item *">About us</a>
						</li>
						<li class="menu-item menu-item-type-post_type menu-item-object-page">
							<a href="#" data-edit="ecode_menu .nav_main_menu .ul_main_menu .menu-item *">Services</a>
						</li>
						<li class="menu-item menu-item-type-post_type menu-item-object-page">
							<a href="#" data-edit="ecode_menu .nav_main_menu .ul_main_menu .menu-item *">Blog</a>
						</li>
						<li class="menu-item menu-item-type-post_type menu-item-object-page">
							<a href="#" data-edit="ecode_menu .nav_main_menu .ul_main_menu .menu-item *">About</a>
						</li>
					</ul>
				</nav>{{/builder}}
			</section>
			...
			?>{{/php}}
			<section class="ecode_section_footer ecode_contact">
				<p class="ecode_title"{{builder}} data-edit="ecode_title"{{/builder}}>{{copy_contact}}</p>
				<div class="ecode_contact_form">
					...
					<div role="form" class="wpcf7" id="wpcf7-f75-o1" lang="es-ES" dir="ltr">
						...
						<form action="#" method="post" class="wpcf7-form init" novalidate="novalidate" data-status="init">
							...
							<div class="container_button">
								<p class="wpcf7-form-control wpcf7-submit button button_primary" data-edit="ecode_contact .container_button .button">Submit</p>
							</div>
		...
		<div class="ecode_section_copyright">
			{{php}}<?php

			if( !empty( $footer_info ) ) {

			?>{{/php}}
			<p class="ecode_copyright">{{footer_info}}</p>
			{{php}}<?php
		...
		?>{{/php}}
		<div id="ecode_tabbar_whatsapp_92_108" class="ecode_tabbar_whatsapp_92_108">
