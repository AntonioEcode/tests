...
<section id="page_section_{{page_section_id}}"{{builder}} class="page_section"{{/builder}}>
	<section class="ecode_section_47_template_55">
		<div class="ecode_triangle_left"{{builder}} data-edit-center="ecode_triangle_left::after"{{/builder}}></div>
		<div class="ecode_triangle_right"{{builder}} data-edit-center="ecode_triangle_right::after"{{/builder}}></div>
		<section class="ecode_articles">
			...
			?>{{/php}}
			<section id="ecode_slider_47_55" class="ecode_slider_47_55">
		...
		if( !empty( $data->featured_pretitle ) || !empty( $data->featured_title ) || !empty( $data->featured_desc ) ) {

		?>{{/php}}
		<section class="ecode_featured"{{builder}} data-edit-top-right="ecode_featured"{{/builder}}>
			{{php}}<?php

			if( !empty( $data->featured_image_hd_src ) && !empty( $data->featured_image_src ) ) {

			?>{{/php}}
			<figure class="ecode_featured_figure"{{builder}} data-edit="ecode_featured_figure"{{/builder}}>
				{{php}}<?php

				if( !empty( $data->featured_image_hd_src ) ) {
				...
				?>{{/php}}
				<img loading="lazy" src="{{featured_image_src}}" alt="{{featured_image_alt}}">
				{{php}}<?php
			...
			<div>
				...
				?>{{/php}}
				<p class="ecode_featured_subtitle"{{builder}} data-edit="ecode_featured_subtitle"{{/builder}}>{{featured_pretitle}}</p>
				{{php}}<?php

				}

				if( !empty( $data->featured_title ) ) {

				?>{{/php}}
				<h3 class="ecode_featured_title"{{builder}} data-edit="ecode_featured_title"{{/builder}}>{{featured_title}}</h3>
				{{php}}<?php

				}

				if( !empty( $data->featured_desc ) ) {

				?>{{/php}}
				<p class="ecode_featured_text"{{builder}} data-edit="ecode_featured_text"{{/builder}}>{{featured_desc}}</p>
				{{php}}<?php

				}

				if( !empty( $data->featured_button_txt ) && !empty( $data->featured_button_url ) ) {

				?>{{/php}}
				<a href="{{featured_button_url}}" class="ecode_featured_button"{{builder}} data-edit="ecode_featured_button"{{/builder}}>{{featured_button_txt}}</a>
				{{php}}<?php
	...
	<script type="text/javascript">
		...
		for (var i = 0; i < array_s47_t55.length; i++) {
			array_articles = array_s47_t55[i].querySelectorAll( '.ecode_article' );
