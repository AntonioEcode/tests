    /* Section_128 template_194 */

    $works_{{page_section_id}} = wpeb_get_selector_options_by_template( 'work' );

	$cmb = new_cmb2_box( array(
		'id'           => $prefix . 'works_box_{{page_section_id}}',
		'title'        => __( 'Listado de trabajos. Bloques de imagen + título + descripción + enlace', 'cmb2' ) . wpeb_get_section_info( $current_page_id, '{{page_section_id}}', '{{section_id}}', '{{template_id}}' ),
    ...
    $cmb->add_group_field( $group_field_id, array(
        ...
        'desc' => __( 'Descripción del trabajo', 'cmb2' ),
        'id'   => 'card_description',
        'type' => 'textarea'
    ) );
