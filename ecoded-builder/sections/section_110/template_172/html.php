...
$back_img_hd_id  = get_option( 'ecode_back_img_desktop' );
$back_img_hd_src = !empty( $back_img_hd_id ) ? wp_get_attachment_image_src( $back_img_hd_id, 'full' )[0] : NULL;
$image_alt       = !empty( get_post_meta( $back_img_hd_id, '_wp_attachment_image_alt', TRUE ) ) ? get_post_meta( $back_img_hd_id, '_wp_attachment_image_alt', TRUE ) : $logo_alt;

$back_img_id  = get_option( 'ecode_back_img_mobile' );
$back_img_src = !empty( $back_img_id ) ? wp_get_attachment_image_src( $back_img_id, 'full' )[0] : $back_img_hd_src;

?>{{/php}}
<section id="page_section_{{page_section_id}}"{{builder}} class="page_section"{{/builder}}>
	<footer class="ecode_section_110_template_172">
		{{php}}<?php

		if( !empty( $back_img_hd_src ) ) {

		?>{{/php}}
		<picture>
			<source media="(min-width:1024px)" srcset="{{back_img_hd_src}}">
			<img loading="lazy" src="{{back_img_src}}" alt="{{image_alt}}">
		</picture>
		{{php}}<?php
		...
		<div class="ecode_width_110_172">
			...
			?>{{/php}}{{builder}}<div class="ecode_social">
				...
				<a href="#">
					<i><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:svgjs="http://svgjs.com/svgjs" version="1.1" width="512" height="512" x="0" y="0" viewBox="0 0 461.001 461.001" style="enable-background:new 0 0 512 512" xml:space="preserve" class=""><g><path xmlns="http://www.w3.org/2000/svg" style="" d="M365.257,67.393H95.744C42.866,67.393,0,110.259,0,163.137v134.728  c0,52.878,42.866,95.744,95.744,95.744h269.513c52.878,0,95.744-42.866,95.744-95.744V163.137  C461.001,110.259,418.135,67.393,365.257,67.393z M300.506,237.056l-126.06,60.123c-3.359,1.602-7.239-0.847-7.239-4.568V168.607  c0-3.774,3.982-6.22,7.348-4.514l126.06,63.881C304.363,229.873,304.298,235.248,300.506,237.056z" fill="#000000"></g></svg></i>
				</a>
			</div>{{/builder}}
		</div>
		<div class="ecode_section_copyright">
			...
			?>{{/php}}
			<p class="ecode_copyright"{{builder}} data-edit="ecode_copyright"{{/builder}}>{{footer_info}}</p>
			{{php}}<?php
		...
		?>{{/php}}
		<div id="ecode_tabbar_whatsapp_110_172" class="ecode_tabbar_whatsapp_110_172">
