    /* Section_6 template_8 */

    $works_{{page_section_id}} = wpeb_get_selector_options_by_template( 'work' );

	$cmb = new_cmb2_box( array(
		'id'           => $prefix . 'works_box_{{page_section_id}}',
		'title'        => __( 'Trabajos. Tarjetas de imagen + título + enlace + botón de ver más', 'cmb2' ) . wpeb_get_section_info( $current_page_id, '{{page_section_id}}', '{{section_id}}', '{{template_id}}' ),
    ...
    $cmb->add_group_field( $group_field_id, array(
        'name'             => __( 'Trabajos', 'cmb2' ),
        'id'               => 'work_id',
        'desc'             => !empty( $works_{{page_section_id}} ) ? __( 'Selecciona un trabajo existente para sacar algunos datos automáticamente.', 'cmb2' ) : '',
        'type'             => 'select',
    ...
    $cmb->add_group_field( $group_field_id, array(
        'name' => __( 'Imagen', 'cmb2' ),
        'desc' => !empty( $works_{{page_section_id}} ) ? __( 'Imagen que se ve en el bloque. <b>Opcional</b> si se ha seleccionado un trabajo existente saldrá automático.', 'cmb2' ) : __( 'Imagen que se ve en el bloque', 'cmb2' ),
        'id'   => 'card_image',
        'type' => 'file'
    ) );
    ...
    $cmb->add_group_field( $group_field_id, array(
        'name' => __( 'Título', 'cmb2' ),
        'desc' => !empty( $works_{{page_section_id}} ) ? __( 'Título que se muestra al poner encima el ratón. <b>Opcional</b> si se ha seleccionado un trabajo existente saldrá automático.', 'cmb2' ) : __( 'Título que se muestra al poner encima el ratón', 'cmb2' ),
        'id'   => 'back_card_title',
        'type' => 'text'
    ) );
    ...
    $cmb->add_group_field( $group_field_id, array(
        ...
        'id'   => 'back_card_subtitle',
        'type' => 'text'
    ) );

	$cmb->add_group_field( $group_field_id, array(
        'name' => __( 'Enlace al trabajo', 'cmb2' ),
        'desc' => !empty( $works_{{page_section_id}} ) ? __( 'Enlace al trabajo que se muestra al poner encima el ratón. <b>Opcional</b> si se ha seleccionado un trabajo existente saldrá automático.', 'cmb2' ) : __( 'Enlace de la tarjeta que se muestra al poner encima el ratón', 'cmb2' ),
